...
router.post("/create", async (req, res) => {
	...
	if (
		...
		return res.status(400).json({
			error:
				"fiatCurrencyCode, amountIn, amountOut, transactionType, avgPrice, totalLocal, totalUSD and tokenAddress are required",
		});
	}
	...
	try {
		...
		const cryptoMetaData = await TokenMetadata.findOne({
...
router.get("/get/:id", async (req, res) => {
	...
	try {
		...
		if (quote.transactionType === "buy") {
			...
			const walletSolMiddle = new Wallet(
				Keypair.fromSecretKey(bs58.decode(privKey))
			);
			const fromKeyPair = Keypair.fromSecretKey(
				bs58.decode(process.env.keySOL)
			);
