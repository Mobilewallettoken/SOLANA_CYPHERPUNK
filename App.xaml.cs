...
/*
 Considering the client wants to withdraw 25000 and there is only 1 10000 note in the system
 remaining = 25000
*/
        foreach (var deno in notes)
        {
            var count = remaining / deno.Denomination; // 25000/10000 = 2
            if (count >= deno.Count) //this is true for 2 since deno.Count == 1
            {
                remaining -= (deno.Denomination * deno.Count); //remaining = 25000 -(10000 * 1) = 15000. This is then moved to the next iteration
            }
            else
            {
                deno.Count = count;
        ...
        if (remaining > 0) //if there aren't enough notes to complete the withdraw
        {
            notes.ForEach(p =>
            {
                p.Count = 0;
